...
from is3107.dump_track_info import dump_trackinfo_op                                #Task16
...
with DAG(
    ...
    def extract_new_releases(**kwargs):
        ...
        with open(f'{path}/airflow/dags/intermediate/albums.json', 'w', encoding='utf-8') as f:
    ...
    def clean_new_albums(**kwargs):
        ...
        with open(f'{path}/airflow/dags/intermediate/albums.json', 'r') as f:
            albums = json.load(f)
        clean_albums = clean_new_albums_op(albums, extract_date_album)
        clean_albums.to_csv(f'{path}/airflow/dags/intermediate/clean_new_albums.csv', index=False)
        print("ALBUM CLEANED")
    ...
    def extract_new_albums_full_info(**kwargs):
        ti = kwargs['ti']
        clean_albums = pd.read_csv(f'{path}/airflow/dags/intermediate/clean_new_albums.csv')
        ...
        with open(f'{path}/airflow/dags/intermediate/albums_full_info.json', 'w', encoding='utf-8') as f2:
    ...
    def get_new_track_ids(**kwargs):
        ...
        clean_albums['release_date'] = clean_albums['release_date'].apply(lambda x: parser.parse(x).strftime('%Y-%m-%d'))
        clean_albums['extract_date'] = clean_albums['extract_date'].apply(lambda x: parser.parse(x).strftime('%Y-%m-%d'))
        with open (f'{path}/airflow/dags/intermediate/albums_full_info.json', 'r') as f2:
            albums_full_info = json.load(f2)
        new_track_ids = get_new_track_ids_op(clean_albums, albums_full_info)
        new_track_ids.to_csv(f'{path}/airflow/dags/intermediate/new_track_ids.csv', index=False)
    ...
    def extract_new_track_info(**kwargs):
        ti = kwargs['ti']
        new_track_ids = pd.read_csv(f'{path}/airflow/dags/intermediate/new_track_ids.csv')
        ...
        with open(f'{path}/airflow/dags/intermediate/new_tracks.json', 'w', encoding='utf-8') as f:
    ...
    def clean_new_track_info(**kwargs):
        ...
        new_track_ids = pd.read_csv(f'{path}/airflow/dags/intermediate/new_track_ids.csv')
        new_track_ids['artists_id'] = new_track_ids['artists_id'].apply(eval)
        with open(f'{path}/airflow/dags/intermediate/new_tracks.json', 'r') as f:
            new_tracks = json.load(f)
        clean_new_tracks = clean_new_track_info_op(new_tracks, new_track_ids, extract_date_track)
        clean_new_tracks.to_csv(f'{path}/airflow/dags/intermediate/clean_new_tracks.csv', index=False)
    ...
    def extract_audio_features(**kwargs):
        ...
        with open(f'{path}/airflow/dags/intermediate/audio_features.json', 'w', encoding='utf-8') as f:
    ...
    def clean_audio_features(**kwargs):
        ti = kwargs['ti']
        with open(f'{path}/airflow/dags/intermediate/audio_features.json', 'r') as f:
            audio_features = json.load(f)
        clean_audio_features = clean_audio_features_op(audio_features)
        clean_audio_features.to_csv(f'{path}/airflow/dags/intermediate/clean_audio_features.csv', index=False)
    ...
    def extract_artist(**kwargs):
        ...
        with open(f'{path}/airflow/dags/intermediate/new_artists.json', 'w', encoding='utf-8') as f:
    ...
    def clean_artist(**kwargs):
        ti = kwargs['ti']
        with open(f'{path}/airflow/dags/intermediate/new_artists.json', 'r') as f:
            new_artists = json.load(f)
        clean_new_artists = clean_artist_op(new_artists)
        clean_new_artists.to_csv(f'{path}/airflow/dags/intermediate/clean_new_artists.csv', index=False)
    ...
    def load_new_track_info(**kwargs):
        ti = kwargs['ti']
        clean_new_tracks = pd.read_csv(f'{path}/airflow/dags/intermediate/clean_new_tracks.csv')
    ...
    def load_audio_features(**kwargs):
        ti = kwargs['ti']
        clean_audio_features = pd.read_csv(f'{path}/airflow/dags/intermediate/clean_audio_features.csv')
    ...
    def load_artist(**kwargs):
        ti = kwargs['ti']
        clean_new_artists = pd.read_csv(f'{path}/airflow/dags/intermediate/clean_new_artists.csv')
